...
\title{The Mathematics of the Sequence Connectivity Engine}
\author{Sequence}
\date{July 2026}
...
\begin{abstract}
Sequence is a word game: connect a start word to an end word through a chain of
intermediate words, where every adjacent pair must be semantically related, and
the shortest verified chain wins. This note formalizes the mathematics behind
...
A Sequence puzzle is a pair of words $(s, e)$. A player answers with a chain of
...
threshold, yet the entity \texttt{Star\_Wars} exists as its own vocabulary
entry. The predicate
\begin{gather*}
  \mathrm{colloc}(a, b) \iff
  \text{some cased joining of $a, b$ (either order)} \\
  \text{is a vocabulary entry.}
\end{gather*}
...
The threshold is not chosen a priori; it is measured. Fix a must-pass set $P$
...
Take a puzzle $(s, e)$ with connectors $c_1, \dots, c_k$, and set $c_0 = s$
and $c_{k+1} = e$. The sequence verifies iff
\begin{enumerate}
